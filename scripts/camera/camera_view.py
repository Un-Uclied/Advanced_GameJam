...
class CameraView:
    @staticmethod
    def world_rect_to_screen_rect(camera: Camera2D, rect: pg.Rect) -> pg.Rect:
        '''
    ...
    @staticmethod
    def is_rect_in_view(camera: Camera2D, rect: pg.Rect) -> bool:
        '''
        주어진 월드 Rect가 카메라 뷰포트(화면)에 보이는지 체크

        :param camera: 현재 카메라 객체 (보통 self.camera)
        :param rect: 월드 좌표 기준 Rect (절대 좌표)
        :return: 화면에 보이면 True, 아니면 False
        '''
        screen_rect = CameraView.world_rect_to_screen_rect(camera, rect)
